...
class Matrix {
		...
		public:
				...
				Matrix operator-(Matrix& other) {
						...
						for (int i = 0; i < rows; i++) {
								for (int j = 0; j < cols; j++) {
										sum[i][j] = (((matrix[i][j] - other[i][j]) % MOD + MOD) % MOD);
								}
						}
						return Matrix(sum);
				}
						...
						for (int i = 0; i < rows; i++) {
								for (int j = 0; j < other.rows; j++) {
										for (int k = 0; k < other.cols; k++) {
												product[i][j] = (product[i][k] + 1ll * matrix[i][j] * other[j][k]) % MOD;
...
//												if (product[i][k] >= 1ll * MOD * MOD) product[i][k] -= 1ll*MOD*MOD;
				...
				Matrix& operator*=(Matrix& other) {
						...
						for (int i = 0; i < rows; i++) {
								for (int j = 0; j < other.rows; j++) {
										for (int k = 0; k < other.cols; k++) {
												product[i][k] = (product[i][k] + 1ll * matrix[i][j] * other[j][k]) % MOD;
										}
								}
						}
						for (int i = 0; i < rows; i++) {
...
int main() {
		...
		vector<vector<long long>> A(x, vector<long long>(x, 0));

		map<ll, ll> freq;
		ll v;
		F0R(i, n) cin >> v, freq[v]++;
